...
def Chorin_splitting_with_pressure_correction(space_disc: SpaceDiscretisation,
    ...
    L0 = Lambda*dW/(tau*2.0)*inner(dot(grad(utildeOld), as_vector([1,1])), grad(q))*dx

    #variational form: Helmholtz-projected noise
    a1 = inner(u,v)*dx
    L1 = ( Lambda*dW/(tau*2.0)*inner(dot(grad(utildeOld), as_vector([1,1])), v) -inner(grad(psto),v) )*dx
