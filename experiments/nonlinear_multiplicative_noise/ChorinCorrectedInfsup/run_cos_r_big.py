...
from local_src.algorithm import Chorin_splitting_with_pressure_correction
...
def generate_one(time_disc: TimeDiscretisation,
    ...
    ref_to_time_to_pressureSTO = dict()
    ref_to_time_to_pressureDET = dict()
    for level in time_disc.refinement_levels:
        ...
        (ref_to_time_to_velocity[level],
         ...
         ref_to_time_to_pressureSTO[level],
         ref_to_time_to_pressureDET[level])  = algorithm(
    ...
    return (noise_coeff_to_ref_to_noise_increments,
            ...
            ref_to_time_to_pressureSTO,
            ref_to_time_to_pressureDET)
...
def generate() -> None:
    ...
    ### noise coefficient
    noise_coefficients = COS_BASIS(gcf.TRUNCATION_INDEX_NOISE,space_disc.mesh,space_disc.velocity_space,cf.R_VALUE_NOISE)
    ...
    if gcf.TIME_CONVERGENCE:
        ...
        time_convergence_pressureSTO = ProcessManager([
            TimeComparison(time_disc.ref_to_time_stepsize,"L2_L2_pressureSTO",l2_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"H-1_L2_pressureSTO",h_minus1_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"W-1_inf_L2_pressureSTO",w_minus1_inf_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            ])
        time_convergence_pressureDET = ProcessManager([
            TimeComparison(time_disc.ref_to_time_stepsize,"L2_L2_pressureDET",l2_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"H-1_L2_pressureDET",h_minus1_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"W-1_inf_L2_pressureDET",w_minus1_inf_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            ])
        ...
        time_convergence_pressureSTO_weak = ProcessManager([
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_kin_pressureSTO",end_time_X_distance,kinetic_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_sin_pressureSTO",end_time_X_distance,SIN_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_kin_IntpressureSTO",end_time_integrated_X_distance,kinetic_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_sin_IntpressureSTO",end_time_integrated_X_distance,SIN_distance,gcf.TIME_COMPARISON_TYPE),
            ])
        time_convergence_pressureDET_weak = ProcessManager([
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_kin_pressureDET",end_time_X_distance,kinetic_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_sin_pressureDET",end_time_X_distance,SIN_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_kin_IntpressureDET",end_time_integrated_X_distance,kinetic_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparisonWeak(time_disc.ref_to_time_stepsize,"Weak_sin_IntpressureDET",end_time_integrated_X_distance,SIN_distance,gcf.TIME_COMPARISON_TYPE),
            ])
    ...
    if gcf.STABILITY_CHECK:
        ...
        stability_check_pressureSTO = ProcessManager([
            StabilityCheck(time_disc.ref_to_time_stepsize,"L2_L2_pressureSTO",l2_X_norm,l2_space),
            StabilityCheck(time_disc.ref_to_time_stepsize,"H-1_L2_pressureSTO",h_minus1_X_norm,l2_space)
        ])
        stability_check_pressureDET = ProcessManager([
            StabilityCheck(time_disc.ref_to_time_stepsize,"L2_L2_pressureDET",l2_X_norm,l2_space),
            StabilityCheck(time_disc.ref_to_time_stepsize,"H-1_L2_pressureDET",h_minus1_X_norm,l2_space)
        ])
    ...
    if gcf.STATISTICS_CHECK:
        ...
        statistics_pressureSTO = StatisticsObject("pressureSTO",time_disc.ref_to_time_grid,space_disc.pressure_space)
        statistics_pressureDET = StatisticsObject("pressureDET",time_disc.ref_to_time_grid,space_disc.pressure_space)
    ...
    for k in new_seeds:
        ...
        (ref_to_noise_increments, 
         ...
         ref_to_time_to_pressure,
         ref_to_time_to_pressureSTO,
         ref_to_time_to_pressureDET) = generate_one(time_disc=time_disc,
                                                           space_disc=space_disc,
                                                           noise_coefficients=noise_coefficients,
                                                           initial_velocity=initial_velocity,
                                                           algorithm=Chorin_splitting_with_pressure_correction,
        ...
        if gcf.TIME_CONVERGENCE:
            ...
            time_to_fine_pressureSTO = ref_to_time_to_pressureSTO[time_disc.refinement_levels[-1]]
            time_to_fine_pressureDET = ref_to_time_to_pressureDET[time_disc.refinement_levels[-1]]
            
            time_convergence_pressure.update(ref_to_time_to_pressure,time_to_fine_pressure)
            time_convergence_pressureSTO.update(ref_to_time_to_pressureSTO,time_to_fine_pressureSTO)
            time_convergence_pressureDET.update(ref_to_time_to_pressureDET,time_to_fine_pressureDET)
            
            time_convergence_pressure_weak.update(ref_to_time_to_pressure,time_to_fine_pressure)
            time_convergence_pressureSTO_weak.update(ref_to_time_to_pressureSTO,time_to_fine_pressureSTO)
            time_convergence_pressureDET_weak.update(ref_to_time_to_pressureDET,time_to_fine_pressureDET)
        ...
        if gcf.STABILITY_CHECK:
            ...
            stability_check_pressureSTO.update(ref_to_time_to_pressureSTO)
            stability_check_pressureDET.update(ref_to_time_to_pressureDET)
        ...
        if gcf.STATISTICS_CHECK:
            ...
            statistics_pressureSTO.update(ref_to_time_to_pressureSTO)
            statistics_pressureDET.update(ref_to_time_to_pressureDET)
    ...
    if gcf.TIME_CONVERGENCE:
        ...
        logging.info(time_convergence_pressureSTO)
        logging.info(time_convergence_pressureDET)
        time_convergence_velocity.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressure.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressureSTO.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressureDET.save(cf.TIME_DIRECTORYNAME)
        logging.info(time_convergence_velocity_weak)
        logging.info(time_convergence_pressure_weak)
        logging.info(time_convergence_pressureSTO_weak)
        logging.info(time_convergence_pressureDET_weak)
        time_convergence_velocity_weak.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressure_weak.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressureSTO_weak.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressureDET_weak.save(cf.TIME_DIRECTORYNAME)
    ...
    if gcf.STABILITY_CHECK:
        ...
        logging.info(stability_check_pressureSTO)
        logging.info(stability_check_pressureDET)
        stability_check_velocity.save(cf.STABILITY_DIRECTORYNAME)
        stability_check_pressure.save(cf.STABILITY_DIRECTORYNAME)
        stability_check_pressureSTO.save(cf.STABILITY_DIRECTORYNAME)
        stability_check_pressureDET.save(cf.STABILITY_DIRECTORYNAME)
    ...
    if gcf.STATISTICS_CHECK:
        ...
        statistics_pressureSTO.save(cf.VTK_DIRECTORY + "/" + cf.STATISTICS_DIRECTORYNAME)
        statistics_pressureDET.save(cf.VTK_DIRECTORY + "/" + cf.STATISTICS_DIRECTORYNAME)
