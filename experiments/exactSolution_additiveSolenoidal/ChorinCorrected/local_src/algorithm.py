...
def Chorin_splitting(space_disc: SpaceDiscretisation,
    ...
    L1 = ( inner(uold,v) - 1.0/Re*tau*inner(bodyforce1,v)*(1+W) + 2*tau*t*inner(bodyforce2,v) + dW*inner(noise_coefficient,v) )*dx
...
def Chorin_splitting_with_pressure_correction(space_disc: SpaceDiscretisation,
    ...
    L0 = dW/tau*inner(noise_coefficient,grad(q))*dx

    #variational form: Helmholtz-projected noise
    a1 = inner(u,v)*dx
    L1 = (dW/tau*inner(noise_coefficient,v)  -inner(grad(psto),v) )*dx
