...
def generate_one(time_disc: TimeDiscretisation,
    ...
    ref_to_time_to_stochastic_pressure = dict()
    ref_to_time_to_deterministic_pressure = dict()
    for level in time_disc.refinement_levels:
        ...
        (ref_to_time_to_velocity[level],
         ...
         ref_to_time_to_stochastic_pressure[level],
         ref_to_time_to_deterministic_pressure[level])  = algorithm(
    ...
    return (noise_coeff_to_ref_to_noise_increments,
            ...
            ref_to_time_to_stochastic_pressure,
            ref_to_time_to_deterministic_pressure)
...
def generate() -> None:
    ...
    if gcf.TIME_CONVERGENCE:
        ...
        time_convergence_stochastic_pressure = ProcessManager([
            TimeComparison(time_disc.ref_to_time_stepsize,"L2_L2_stochastic_pressure",l2_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"H-1_L2_stochastic_pressure",h_minus1_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"W-1_inf_L2_stochastic_pressure",w_minus1_inf_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE)
            ])
        time_convergence_deterministic_pressure = ProcessManager([
            TimeComparison(time_disc.ref_to_time_stepsize,"L2_L2_deterministic_pressure",l2_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"H-1_L2_deterministic_pressure",h_minus1_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE),
            TimeComparison(time_disc.ref_to_time_stepsize,"W-1_inf_L2_deterministic_pressure",w_minus1_inf_X_distance,l2_distance,gcf.TIME_COMPARISON_TYPE)
            ])
    ...
    if gcf.STABILITY_CHECK:
        ...
        stability_check_stochastic_pressure = ProcessManager([
            StabilityCheck(time_disc.ref_to_time_stepsize,"L2_L2_stochastic_pressure",l2_X_norm,l2_space),
            StabilityCheck(time_disc.ref_to_time_stepsize,"H-1_L2_stochastic_pressure",h_minus1_X_norm,l2_space)
        ])
        stability_check_deterministic_pressure = ProcessManager([
            StabilityCheck(time_disc.ref_to_time_stepsize,"L2_L2_deterministic_pressure",l2_X_norm,l2_space),
            StabilityCheck(time_disc.ref_to_time_stepsize,"H-1_L2_deterministic_pressure",h_minus1_X_norm,l2_space)
        ])
    ...
    if gcf.STATISTICS_CHECK:
        ...
        statistics_stochastic_pressure = StatisticsObject("stochastic-pressure",time_disc.ref_to_time_grid,space_disc.pressure_space)
        statistics_deterministic_pressure = StatisticsObject("deterministic-pressure",time_disc.ref_to_time_grid,space_disc.pressure_space)
    ...
    for k in new_seeds:
        ...
        (ref_to_noise_increments, 
         ...
         ref_to_time_to_pressure,
         ref_to_time_to_stochastic_pressure,
         ref_to_time_to_deterministic_pressure) = generate_one(time_disc=time_disc,
                                                           space_disc=space_disc,
        ...
        if gcf.TIME_CONVERGENCE:
            ...
            #stochastic pressure
            time_to_fine_stochastic_pressure = ref_to_time_to_stochastic_pressure[time_disc.refinement_levels[-1]]
            time_convergence_stochastic_pressure.update(ref_to_time_to_stochastic_pressure,time_to_fine_stochastic_pressure)
            #deterministic pressure
            time_to_fine_deterministic_pressure = ref_to_time_to_deterministic_pressure[time_disc.refinement_levels[-1]]
            time_convergence_deterministic_pressure.update(ref_to_time_to_deterministic_pressure,time_to_fine_deterministic_pressure)
        ...
        if gcf.STABILITY_CHECK:
            ...
            stability_check_stochastic_pressure.update(ref_to_time_to_stochastic_pressure)
            stability_check_deterministic_pressure.update(ref_to_time_to_deterministic_pressure)
        ...
        if gcf.STATISTICS_CHECK:
            ...
            statistics_stochastic_pressure.update(ref_to_time_to_stochastic_pressure)
            statistics_deterministic_pressure.update(ref_to_time_to_deterministic_pressure)
    ...
    if gcf.TIME_CONVERGENCE:
        ...
        logging.info(time_convergence_stochastic_pressure)
        logging.info(time_convergence_deterministic_pressure)
        time_convergence_velocity.save(cf.TIME_DIRECTORYNAME)
        time_convergence_pressure.save(cf.TIME_DIRECTORYNAME)
        time_convergence_stochastic_pressure.save(cf.TIME_DIRECTORYNAME)
        time_convergence_deterministic_pressure.save(cf.TIME_DIRECTORYNAME)
    ...
    if gcf.STABILITY_CHECK:
        ...
        logging.info(stability_check_stochastic_pressure)
        logging.info(stability_check_deterministic_pressure)
        stability_check_velocity.save(cf.STABILITY_DIRECTORYNAME)
        stability_check_pressure.save(cf.STABILITY_DIRECTORYNAME)
        stability_check_stochastic_pressure.save(cf.STABILITY_DIRECTORYNAME)
        stability_check_deterministic_pressure.save(cf.STABILITY_DIRECTORYNAME)
    ...
    if gcf.STATISTICS_CHECK:
        ...
        statistics_stochastic_pressure.save(cf.VTK_DIRECTORY + "/" + cf.STATISTICS_DIRECTORYNAME)
        statistics_deterministic_pressure.save(cf.VTK_DIRECTORY + "/" + cf.STATISTICS_DIRECTORYNAME)
