...
def _compute_weakError(iter_of_float) -> float:
    """Compute the weak error."""
    return absolute(mean_value(iter_of_float))
...
class TimeComparisonWeak(ProcessObject):
    ...
    @property
    def ref_to_weakError(self) -> dict[int,float]:
        return {level: _compute_weakError(self.ref_to_seed_to_error[level].values()) for level in self.ref_to_seed_to_error.keys()}
    
    @property
    def ref_to_EOC_weakError(self) -> dict[int,float]:
        return get_ref_to_EOC(self.ref_to_weakError,self.ref_to_stepsize)
    ...
    def save(self,name_directory) -> None:
        ...
        header = ["dt","weakError","EOC","Standard_Deviation"]
        data = []
        for level in list(self.ref_to_stepsize.keys())[:-1]:
            data.append([self.ref_to_stepsize[level], self.ref_to_weakError[level],self.ref_to_EOC_weakError[level],self.ref_to_error_deviation[level]])
    ...
    def __str__(self):
        ...
        eoc_message += "\n|__dt__|\t|Weak Error|_EOC|\t|_DEVIATION|"  
        for level in list(self.ref_to_stepsize.keys())[:-1]:
            eoc_message += f"\n|{self.ref_to_stepsize[level]:5.04f}|\t"
            eoc_message += f"|{self.ref_to_weakError[level]:10.08f}|{self.ref_to_EOC_weakError[level]:3.02f}|\t"
