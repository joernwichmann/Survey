...
def get_algorithm_by_name(algorithm_name: str) -> StokesAlgorithm:
    match algorithm_name:
        ...
        case "modified Chorin splitting":
            return Chorin_splitting_with_pressure_correction
...
def Chorin_splitting(space_disc: SpaceDiscretisation,
    ...
    L1 = ( inner(uold,v) + tau*inner(det_forcing,v) )*dx
    for noise_coeff in noise_coeff_to_noise_increments:
        L1 = L1 + noise_coeff_to_dW[noise_coeff]*inner(noise_coeff, v)*dx
...
def Chorin_splitting_with_pressure_correction(space_disc: SpaceDiscretisation,
    ...
    for noise_coeff in noise_coeff_to_noise_increments:
        L0 = L0 + 1/tau*noise_coeff_to_dW[noise_coeff]*inner(noise_coeff, grad(q))*dx

    #variational form: Helmholtz-projected noise
    a1 = inner(u,v)*dx
    L1 = -inner(grad(psto),v)*dx
    for noise_coeff in noise_coeff_to_noise_increments:
        L1 = L1 + 1/tau*noise_coeff_to_dW[noise_coeff]*inner(noise_coeff, v)*dx

    #variational form: artificial velocity
    a2 = ( inner(u,v) + 1/Re*tau*inner(grad(u), grad(v)) )*dx
    L2 = ( inner(uold,v) + tau*inner(det_forcing,v) + tau*inner(noise_projected,v))*dx
...
#####################################################
# Algorithms for Sublinear Multiplicative Noise

def nonlinearity(u: Function) -> Function:
    '''Input: VectorElement
    Output: VectorElement'''
    return ( (u[0]**2 +1)**(1/2.0), (u[1]**2 +1)**(1/2.0) )
